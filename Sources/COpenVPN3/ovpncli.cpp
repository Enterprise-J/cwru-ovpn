...
#if !defined(CWRU_OVPN_NATIVE_DNS_DISABLED)
#error "Apply scripts/patches/openvpn3-cwru-policy.patch to the pinned OpenVPN 3 checkout."
#endif
...
class ClientState
{
  public:
    ...
    {
        foreign_thread_ready.store(true);
        if (foreign_thread_stop_pending.exchange(false))
            trigger_async_stop_local();
    }

    void request_stop_from_foreign_thread()
    {
        foreign_thread_stop_pending.store(true);
        if (foreign_thread_ready.load() && foreign_thread_stop_pending.exchange(false))
            trigger_async_stop_local();
    }
  ...
  private:
    ...
    std::atomic<bool> foreign_thread_stop_pending{false};
};
}; // namespace Private
...
{
    state->request_stop_from_foreign_thread();
}
