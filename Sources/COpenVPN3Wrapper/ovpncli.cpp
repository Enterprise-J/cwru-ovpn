...
// Keep this vendored copy intentionally close to upstream and prefer small,
// well-contained changes over broad rewrites.
...
OPENVPN_CLIENT_EXPORT void OpenVPNClientHelper::parse_config(const Config &config, EvalConfig &eval, OptionList &options)
{
    try
    {
        ...
        const auto &server_list = cc.serverList();
        eval.serverList.reserve(server_list.size());
        for (const auto &server : server_list)
        {
            eval.serverList.push_back(ServerEntry{server.server, server.friendlyName});
        }
    }
    catch (const std::exception &e)
    {
